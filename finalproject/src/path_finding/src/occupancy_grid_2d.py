    ...
    def SensorCallback(self, msg):
        ...
        for idx, r in enumerate(msg.ranges):
            ...
            for i in range(numpoints):
                p2v = self.PointToVoxel(x * (i/numpoints),y * (i/numpoints))
                if p2v not in points:
                    if p2v[0] >= 0 and p2v[0] < self._x_num and p2v[1] >= 0 and p2v[1] < self._y_num:
                        points.append(p2v)
            if points == []:
                continue
            points = points[::-1]
            occupiedvox = points[0]
            self._map[occupiedvox[0], occupiedvox[1]] = min(self._occupied_threshold, self._map[occupiedvox[0], occupiedvox[1]] + self._occupied_update)

            for vox in points[1:]:
                self._map[vox[0], vox[1]] = max(self._free_threshold, self._map[vox[0], vox[1]] + self._free_update)
